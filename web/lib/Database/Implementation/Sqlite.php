<?php
namespace Yonkoma\Database\Implementation;

use Yonkoma\Database\Database;
use PDO;
use FlagHelper;
use PMCLibrary;
/**
...
class Sqlite implements Database
{
    ...
    public function dbPrepare($reload = false, $transaction = false)
    {
        ...
        if ($transaction) {
            @$this->con->beginTransaction(); // 啟動交易性能模式
        }
    ...
    public function fetchPostList($resno = 0, $start = 0, $amount = 0)
    {
        ...
        if ($resno) { // 輸出討論串的結構 (含自己, EX : 1,2,3,4,5,6)
            ...
            if ($amount) {
                $tmpSQL .= " LIMIT {$start}, {$amount}"; // 指定數量
            }
        }
    ...
    public function isSuccessivePost($lcount, $com, $timestamp, $pass, $passcookie, $host, $isupload)
    {
        ...
        if (!$this->ENV['PERIOD.POST']) {
            return false; // 關閉連續投稿檢查
        }
        $timestamp = intval($timestamp);
        $tmpSQL = 'SELECT pwd,host FROM '.$this->tablename.' WHERE time > '.($timestamp - (int)$this->ENV['PERIOD.POST']); // 一般投稿時間檢查
        if ($isupload) {
            $tmpSQL .= ' OR time > '.($timestamp - (int)$this->ENV['PERIOD.IMAGEPOST']); // 附加圖檔的投稿時間檢查 (與下者兩者擇一)
        } else {
            $tmpSQL .= " OR md5(com) = '".md5($com)."'"; // 內文一樣的檢查 (與上者兩者擇一)
        }
    ...
    public function searchPost($keyword, $field, $method)
    {
        ...
        if ($keyword_cnt > 1) {
            for ($i = 1; $i < $keyword_cnt;
            $i++) {
                $SearchQuery .= " {$method} {$field} LIKE ".$this->con->quote('%'.$keyword[$i].'%'); // 多重字串交集 / 聯集搜尋
            }
        }
